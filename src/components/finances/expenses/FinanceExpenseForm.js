...
import { financeExpenseSchema, photoFileSchema } from "@/schemas/finance-expense-schema";
...
export function FinanceExpenseForm({
    ...
    const isEditMode = !!selectedFinanceExpense
    ...
    useEffect(() => {
        if (isModalOpen) {
            const initialDate = selectedFinanceExpense?.date
                ? new Date(selectedFinanceExpense.date)
                : new Date();

            form.reset({
                date: initialDate,
                finance_category_id: selectedFinanceExpense?.finance_category?.id?.toString() || "",
                description: selectedFinanceExpense?.description || "",
                amount: selectedFinanceExpense?.amount || "",
            })
    ...
    }, [isModalOpen, selectedFinanceExpense, form])
    ...
    const onSubmit = async (values) => {
        try {
            ...
            if (isEditMode) {
                await handleEditFinanceExpense(formData)
            } else {
                await handleAddFinanceExpense(formData)
            }
    ...
    const renderTransactionReceiptPreview = () => {
        ...
        const fileName = fileToPreview.name || selectedFinanceExpense?.transaction_receipt?.split('/').pop() || 'Unknown File';
    ...
    return (
        <ModalForm
            ...
            title={isEditMode ? "Edit Pengeluaran Keuangan" : "Tambah Pengeluaran Keuangan"}
            description={isEditMode ? "Ubah informasi transaksi pengeluaran keuangan yang sudah ada." : "Buat transaksi pengeluaran keuangan baru untuk sistem."}
