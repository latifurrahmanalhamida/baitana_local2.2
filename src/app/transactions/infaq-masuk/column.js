...
export const createColumns = (openEditModal, openDeleteAlert) => [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      ...
      >
        {row.getValue("kode_transaksi")}
      </Badge>
    ),
    size: 180,
  },
  {
    accessorKey: "jenis_infaq.nama",
    header: ({ column }) => (
      <Button
        variant="ghost"
        onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-gray-800 font-medium">
        Rp {Number(row.getValue("jumlah")).toLocaleString("id-ID")}
      </div>
    ),
    size: 150,
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-gray-600">{row.getValue("note") || "-"}</div>
    ),
    size: 300,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const pemasukan = row.original;
      return (
        <div className="flex items-center justify-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => openEditModal(pemasukan)}
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => openDeleteAlert(pemasukan)}
