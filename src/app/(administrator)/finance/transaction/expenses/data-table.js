...
export function DataTable({ columns, data, isLoading = false, searchQuery, onSearchChange, onAddNew }) {
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, columnId, filterValue) => { // filter
            ...
            return (
                row.original.expense_transaction?.toLowerCase().includes(search) ||
                row.original.date_idn_format?.toLowerCase().includes(search) ||
                row.original.finance_category?.name?.toLowerCase().includes(search) ||
                row.original.description?.toLowerCase().includes(search) ||
                row.original.amount_idn_format?.toLowerCase().includes(search) ||
                                ...
                                {table
                                    ...
                                    .map((column) => {
                                        return (
                                            ...
                                            >
                                                {column.id === "expense_transaction"
                                                    ? "Kode Transaksi"
                                                    : column.id === "date"
                                                        ? "Tanggal"
                                                        : column.id === "finance_category"
                                                            ? "Kategori Keuangan"
                                                            : column.id === "description"
                                                                ? "Deskripsi"
                                                                : column.id === "amount_idn_format"
                                                                    ? "Jumlah"
                                                                    : column.id === "transaction_receipt"
                                                                        ? "Bukti Transaksi"
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-32 text-center">
                                        <div className="flex flex-col items-center justify-center space-y-2">
                                            <div className="text-muted-foreground">Tidak ada data transaksi keuangan keluar ditemukan</div>
                                            <div className="text-sm text-muted-foreground">
                                                Coba ubah filter pencarian atau tambah pengeluaran keuangan
                                            </div>
                                        </div>
                                    </TableCell>
                                </TableRow>
